...
input[type=radio]:checked {
    ...
    <body>
        <div class="center">
        <img src="http://localhost:8000/chart" alt='chart'/><br/>
        <h2>Reference: {{ refname }}</h2>
        <h2>Test: {{ testname }}</h2>
        <img src="http://localhost:8000/graph" alt='graph'/>
        <form method="post" action="http://localhost:8000"  enctype="multipart/form-data">
            <input type="file" name="upload" width="980px"><input type="submit" value="Upload">
        </form>   
        </div>
    <footer>
    <i>test upload (c) 2020</i>
    </footer>
    </body>
</html>
